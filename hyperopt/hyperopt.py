...
def evaluation_function(params):
    ...
    dataset.adjust_soft_label(
        params["soft-label-baseline"], params["soft-label-topline"], params["soft-label-baseline-slope"]
    )
    ...
    for i in range(runs):
        ...
        train_loss[i], val_loss[i], acc[i], bacc[i], ece[i], mce[i], auc[i], avg_prec[i], fscore[i], brier[i] = train(
            params, trainset, valset, epochs, is_trial=True, device="cuda"
        )

    result = {
        "train_loss": (train_loss.mean(), stats.sem(train_loss)),
