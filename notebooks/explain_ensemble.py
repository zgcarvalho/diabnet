...
class ExplainModel:
    ...
    def calc_attr(self, age, sex, samples_per_model=10):
        ...
        baseline = torch.stack([self.gen_baselines(sex, age=age) for i in range(len(inputs))]) # same dimension as inputs
        ...
        for m in self.ensemble.models:
            for _ in range(samples_per_model):
                ig = IntegratedGradients(lambda x: torch.sigmoid(m(x)))
