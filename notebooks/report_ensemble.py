...
sns.set()
from sklearn.metrics import roc_auc_score, roc_curve, precision_recall_curve,f1_score, accuracy_score, balanced_accuracy_score, brier_score_loss, average_precision_score
import sys
sys.path.append("../")
...
class DiabNetReport:
    def __init__(self, ensemble, data_suffix, use_negatives=True, use_bmi=False, use_sex=True, use_parents=True):
    ...
    def negatives_older60(self):
        if self._negatives_older60 is None:
            self._negatives_older60 = self.predictor.negatives_life(age_range=range(20,80,5), bmi=-1, samples_per_model=10)
...
######################################################
    ...
    def roc_comp(self, fig_path="", ci=95, bootstrap_samples=1000):
        ...
        idx_sorted0 = rankdata([roc_auc_score(x[:,0], x[:,1]) for x in b0], method='ordinal')
        idx_sorted1 = rankdata([roc_auc_score(x[:,0], x[:,1]) for x in b1], method='ordinal')
        ...
        for i in range(len(idx_sorted0)):
            if idx_sorted0[i] >= idx_min and idx_sorted0[i] <= idx_max:
                ...
                plt.plot(fpr, tpr, color='palegreen', alpha=1, linewidth=.2)
            ...
            if idx_sorted1[i] >= idx_min and idx_sorted1[i] <= idx_max:
                ...
                plt.plot(fpr, tpr, color='lightskyblue', alpha=1, linewidth=.2)
    ...
    def precision_recall_ci_older50(self, ci=95, bootstrap_samples=1000):
        self.precision_recall_CI(self.dataset_test_unique_subset_older50, ci, bootstrap_samples)
    ...
    def precision_recall_CI(db, ci, bootstrap_samples):
        ...
        for i in range(len(idx_sorted)):
            if idx_sorted[i] >= idx_min and idx_sorted[i] <= idx_max:
                ...
                plt.step(recall, precision,where='post', color='gainsboro', alpha=1, linewidth=.2)
                # plt.plot(fpr, tpr, color='gainsboro', alpha=1, linewidth=.2)
    ...
    def precision_recall_comp(self, fig_path="", ci=95, bootstrap_samples=1000):
        ...
        b0 = bootstrap(label_pred0, bootstrap_samples)
        b1 = bootstrap(label_pred1, bootstrap_samples)
        idx_sorted0 = rankdata([average_precision_score(x[:,0], x[:,1]) for x in b0], method='ordinal')
        idx_sorted1 = rankdata([average_precision_score(x[:,0], x[:,1]) for x in b0], method='ordinal')
        ...
        for i in range(len(idx_sorted0)):
            if idx_sorted0[i] >= idx_min and idx_sorted0[i] <= idx_max:
                ...
                plt.step(recall, precision,where='post', color='palegreen', alpha=.2, linewidth=.5)
            if idx_sorted1[i] >= idx_min and idx_sorted1[i] <= idx_max:
                precision, recall, _ = precision_recall_curve(b1[i][:,0], b1[i][:,1])
                plt.step(recall, precision,where='post', color='lightskyblue', alpha=.2, linewidth=.5)
