...
class Dataset:
    ...
    def bootstrap(self, func, num=2000, alpha=0.05, interval="CI"):
        ...
        if interval == "CI":
            return np.mean(data), (
                np.percentile(data, alpha * 100.0 / 2.0),
                np.percentile(data, 100.0 - alpha * 100.0 / 2.0),
            )
        elif interval == "HDI":
            return np.mean(data), self._hdi(data, 1.0 - alpha)
...
class DiabNetReport:
    ...
    def auc(self, interval="CI", bootnum=5000, alpha=0.05):
        ...
        metric = roc_auc_score(db.labels, db.predictions)
        ci = db.bootstrap(roc_auc_score, num=bootnum, alpha=alpha, interval=interval)
        # print(
        #     "AUC = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
            "value": metric,
    ...
    def f1(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = f1_score(db.labels, np.round(db.predictions))
        ci = db.bootstrap(
        ...
        # print(
        #     "F1-Score = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
    ...
    def acc(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = accuracy_score(db.labels, np.round(db.predictions))
        ci = db.bootstrap(
        ...
        # print(
        #     "Accuracy = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
    ...
    def bacc(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = balanced_accuracy_score(db.labels, np.round(db.predictions))
        ci = db.bootstrap(
        ...
        # print(
        #     "Balanced Accuracy = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
    ...
    def precision(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = precision_score(db.labels, np.round(db.predictions))
        ci = db.bootstrap(
    ...
    def sensitivity(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = recall_score(db.labels, np.round(db.predictions), pos_label=1)
        ci = db.bootstrap(
            lambda labels, preds: recall_score(
                db.labels, np.round(db.predictions), pos_label=1
            ),
            num=bootnum,
            alpha=alpha,
            interval=interval,
    ...
    def specificity(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = recall_score(db.labels, np.round(db.predictions), pos_label=0)
        ci = db.bootstrap(
            lambda labels, preds: recall_score(
                db.labels, np.round(db.predictions), pos_label=0
            ),
            num=bootnum,
            alpha=alpha,
            interval=interval,
    ...
    def average_precision(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = average_precision_score(db.labels, db.predictions)
        ci = db.bootstrap(
            average_precision_score, num=bootnum, alpha=alpha, interval=interval
        )
        # print(
        #     "Average Precision Score = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
    ...
    def ece(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = ece(db.labels, db.predictions)
        ci = db.bootstrap(ece, num=bootnum, alpha=alpha, interval=interval)
        # print(
        #     "ECE = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
    ...
    def mce(self, interval="CI", bootnum=5000, alpha=0.05):
        db = self.dataset_test_unique
        metric = mce(db.labels, db.predictions)
        ci = db.bootstrap(mce, num=bootnum, alpha=alpha, interval=interval)
        # print(
        #     "MCE = {:.3f}, mean={:.3f},{}[{:.3f}, {:.3f}]".format(
        #         metric, ci[0], interval, ci[1][0], ci[1][1]
        #     )
        # )
        return {
