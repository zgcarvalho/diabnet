...
def _roc_b(db, ax, num=1000, alpha=0.05, interval="CI", colors=[COLORS[2], "gainsboro"]):
    ...
    ax.text(
        0.7,
        0.05,
        f"{int((1-alpha)*100)}% {interval}: [{aucs[idx[0]]:.3f}, {aucs[idx[-1]]:.3f}]",
        fontsize=13,
...
def _precision_recall_b(db, ax, num=1000, alpha=0.05, interval="CI", colors=[COLORS[2], "gainsboro"]):
    ...
    ax.text(
        0.3,
        0.05,
        f"{int((1-alpha)*100)}% {interval}: [{avgps[idx[0]]:.3f}, {avgps[idx[-1]]:.3f}]",
        fontsize=13,
        horizontalalignment="center",
    )
...
def plot_precision_recall_comp(r, interval="HDI"):
    ...
    _precision_recall_b_comp(
        r.dataset_test_unique, r.dataset_test_unique_subset_neg_older50, ax1, num=1000, alpha=0.05, interval=interval
    )
    return fig
...
def plot_first_positives_lines(r, age_separator=(40, 60)):
    ...
    fig.colorbar(
        mpl.cm.ScalarMappable(norm=norm, cmap=cmap), cax=cbax, orientation="vertical", ticks=np.arange(20, 80, 5)
    )
...
def plot_first_positives_boxplot(r, age_separator=(40, 60)):
    ...
    cbax = fig.add_axes([1.0, 0.13, 0.02, 0.70])
    fig.colorbar(
        mpl.cm.ScalarMappable(norm=norm, cmap=cmap), cax=cbax, orientation="vertical", ticks=np.arange(0, 1.01, 0.1)
    )
...
def plot_core_family(r):
    ...
    for i in range(len(parents)):
        ...
        if True in m_neg_chi:
            plt.plot(np.transpose(predictions[m_neg_chi, :]), color=COLORS[7], ls="--")
            plt.scatter(
                (patient_ages[children_mask] - 20) / 5,
                db.predictions[children_mask],
                color=COLORS[7],
...
def plot_calibration_line(r):
    ...
    fraction_of_positives, mean_predicted_value = calibration_curve(
        r.dataset_test_unique.labels, r.dataset_test_unique.predictions, n_bins=10, strategy="quantile"
    )
    ...
    legend2 = plt.legend(loc="lower right", handles=[ECE, MCE, BRIER])
...
def plot_calibration(r):
    ...
    # Plot calibration curve
    fraction_of_positives, mean_predicted_value = calibration_curve(
        r.dataset_test_unique.labels, r.dataset_test_unique.predictions, n_bins=5, strategy="quantile"
    )
