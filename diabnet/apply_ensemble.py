...
class Predictor(object):
    ...
    def patient(self, feat, age=-1, bmi=-1, samples_per_model=10):
        """
        Parameters:
    ...
    def negatives(self, age=-1, bmi=-1, samples_per_model=10):
        if bmi != -1 and 'BMI' not in self.feat_names:
            print("Warning: BMI is not a feature for this network. Therefore, no impact at predicted values.")
    ...
    def patient_life(self, feat, age_range=range(20,90,5), bmi=-1, samples_per_model=10):
        probs_by_age = []
        for age in age_range:
            probs_by_age.append(self.patient(feat, age, bmi, samples_per_model))
        return probs_by_age, age_range

    def negatives_life(self, age_range=range(20,90,5), bmi=-1, samples_per_model=10):
        if self.negative_data is not None:   
            probs_by_age = []
            for age in age_range:
